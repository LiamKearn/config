...
alias vi = nvim
alias lzg = lazygit
alias lzgc = lazygit --git-dir=($env.HOME)/.cfg --work-tree=($env.HOME)
...
$env.Path = ($env.Path | append $'($env.XDG_DATA_HOME)/cargo/bin')
...
load-env {
  ...
  BROWSER: "chromium",
  LESS: "-RS",
  LESSHISTFILE: $"($env.XDG_STATE_HOME)/less/history",
  MYSQL_HISTFILE: $"($env.XDG_DATA_HOME)/mysql/mysql_history",
  HISTFILE: $"($env.XDG_DATA_HOME)/history",
  CC: "gcc-14",
  CXX: "clang++",
  NODE_REPL_HISTORY: $"($env.XDG_STATE_HOME)/node/repl_history",
  GNUPGHOME: $"($env.XDG_CONFIG_HOME)/gnupg",
  RIPGREP_CONFIG_PATH: $"($env.XDG_CONFIG_HOME)/ripgrep/config",
  PROJECTDIR: $"($env.HOME)/Projects",
  AWS_PROFILE: "personal"
  MBOX: $"($env.HOME)/mbox",
  MAIL: $"($env.HOME)/mbox",
  DOCKER_BUILDKIT: "1",
  HOMEBREW_NO_AUTO_UPDATE: "1",
  PYTHONSTARTUP: $"($env.XDG_CONFIG_HOME)/python/init.py",
  CDK_HOME: $"($env.XDG_DATA_HOME)/cdk",
...
match (sys host).name {
  "Darwin" => {
    load-env {
      LOG_HOME: $"($env.HOME)/Library/Logs",
      LOG_ROOT: "/Library/Logs",
      SDKROOT: (^xcrun --sdk macosx --show-sdk-path | str trim),
      JAVA_HOME: "/opt/homebrew/opt/openjdk@21",
      VCPKG_ROOT: $"($env.HOME)/vcpkg",
      CMAKE_MAKE_PROGRAM: "Ninja"
    }
    $env.LIBRARY_PATH = $"($env.SDKROOT)/usr/lib"
  },
  "Linux" => {
    load-env {
      LOG_HOME: $"($env.XDG_STATE_HOME)",
      LOG_ROOT: "/var/log",
...
# ----------------------
load-env {
  DEFAULTS_CONFIG_DIR: $"($env.XDG_CONFIG_HOME)/defaults",
  GOPATH: $"($env.XDG_DATA_HOME)/go",
  ANDROID_SDK_ROOT: $"($env.HOME)/Library/Android/sdk",
  GRADLE_USER_HOME: $"($env.HOME)/.cache/.gradle",
...
load-env {
  ...
  nvm_default_packages: "yarn pnpm sass aws-cdk",
  GOOGLE_API_KEY: "no",
...
def --wrapped brew [...args] {
    ...
    if (($args | length) < 1 or not ($enabled_commands | any {|cmd| $cmd == $args.0 })) {
        return (^brew ...$args)
    }

    let brewfile_location = $"($env.XDG_CONFIG_HOME)/brew/Brewfile"

    let brew_result = try { ^brew ...$args } | complete

    if ($brew_result.exit_code != 0) {
        log error "Not dumping brewfile, something went wrong."
        return $brew_result.stderr
    }

    let bundle_dump_result = try { ^brew bundle dump --file $brewfile_location --force } | complete
    if ($bundle_dump_result.exit_code != 0) {
        log error "Brew command succeeded but... Failed to dump brewfile:"
        return [$brew_result.stdout, $bundle_dump_result.stderr] | str join "\n"
    }

    log info $"Dumped brewfile to ($brewfile_location)"

    return $brew_result.stdout
}
...
# ctrl-enter to send enter
# I always press ctrl-e to complete history and then enter without releasing ctrl
# $env.config.keybindings ++= [{
#     name: CEnterDOSTUFF
#     modifier: control
#     keycode: Enter
